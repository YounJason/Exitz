---
title: "쿠팡 계정 회원 탈퇴 하는 방법"
serviceName: "쿠팡"
domain: "coupang.com"
pubDate: 2026-08-07
description: "쿠팡 계정 회원 탈퇴 절차, 와우 멤버십 해지, 캐시/캐시백 환불 신청 등 필수 체크리스트와 링크 안내입니다."
logo: "/exit/coupang_1.png"
tabs:
  - id: "pc"
    label: "컴퓨터"
    steps:
      - stepNumber: 0
        title: "시작하기 전에"
        description: "- 쿠팡 와우 멤버십을 구독 중이라면 앱 탈퇴 전 멤버십을 해지해야 합니다. [자세히 알아보기](/exit/coupang-wow)\n- 진행 중인 주문, 배송, 반품, 교환 건이 모두 완료 상태여야 탈퇴가 가능하며, 남은 쿠페이 머니는 계좌 환불을 먼저 진행해야 합니다."
      - stepNumber: 1
        title: "쿠팡 홈페이지 접속 및 마이 쿠팡 클릭"
        description: "쿠팡 웹사이트에 접속하여 로그인을 진행한 후, 메인 페이지 우측 상단 프로필 옆에 위치한 '마이 쿠팡'을 클릭하세요."
        image: "/exit/coupang_2.png"
      - stepNumber: 2
        title: "'MY 정보' - '회원 탈퇴' 메뉴 선택"
        description: "마이 쿠팡 페이지 좌측 사이드바 하단의 'MY 정보' 카테고리를 찾으신 후, 해당 카테고리 내 '회원 탈퇴' 메뉴를 선택하세요."
        actionUrl: "https://login.coupang.com/login/secession.pang"
        actionText: "쿠팡 탈퇴 직행 페이지"
        image: "/exit/coupang_3.png"
      - stepNumber: 3
        title: "탈퇴 유의사항 동의 및 본인 확인"
        description: "회원 탈퇴에 따른 안내 사항(구매 내역 삭제, 포인트 소멸 등)을 신중히 확인한 후 동의 체크박스에 체크하고, 보안을 위한 본인 계정 비밀번호를 입력한 뒤 '다음' 버튼을 누르세요."
        image: "/exit/coupang_4.png"
      - stepNumber: 4
        title: "이용 내역 및 잔여 혜택 최종 확인"
        description: "현재 소지 중인 쿠폰, 캐시, 최근 구매 내역 등이 최종 정리되었는지 확인합니다. 미정리 항목이 없다면 아래 탈퇴 동의 항목에 체크합니다."
        image: "/exit/coupang_5.png"
      - stepNumber: 5
        title: "탈퇴 완료"
        description: "하단의 '탈퇴하기' 버튼을 누르면 쿠팡 계정 영구 탈퇴가 완료됩니다."
        image: "/exit/coupang_6.png"

  - id: "mobile"
    label: "모바일"
    steps:
      - stepNumber: 0
        title: "시작하기 전에"
        description: "- 쿠팡 와우 멤버십을 구독 중이라면 앱 탈퇴 전 멤버십을 해지해야 합니다. [자세히 알아보기](/exit/coupang-wow)\n- 진행 중인 주문, 배송, 반품, 교환 건이 모두 완료 상태여야 탈퇴가 가능하며, 남은 쿠페이 머니는 계좌 환불을 먼저 진행해야 합니다."
      - stepNumber: 1
        title: "쿠팡 앱 실행 및 사용자 아이콘 터치"
        description: "스마트폰에서 쿠팡 앱을 실행한 후, 화면 최하단 네비게이션 바의 사용자 아이콘을 터치하세요."
        image: "/exit/coupang_7.jpg"
      - stepNumber: 2
        title: "설정 진입"
        description: "화면 상단에 우측 상단 설정 버튼을 터치하세요."
        image: "/exit/coupang_8.jpg"
      - stepNumber: 3
        title: "'회원 탈퇴' 메뉴 선택"
        description: "최하단 '회원 탈퇴' 메뉴를 터치하세요."
        image: "/exit/coupang_9.jpg"
      - stepNumber: 4
        title: "탈퇴 유의사항 동의 및 본인 확인"
        description: "회원 탈퇴에 따른 안내 사항(구매 내역 삭제, 포인트 소멸 등)을 신중히 확인한 후 동의 체크박스에 체크하고, 보안을 위한 본인 계정 비밀번호를 입력한 뒤 '다음' 버튼을 누르세요."
        image: "/exit/coupang_10.jpg"
      - stepNumber: 5
        title: "이용 내역 및 잔여 혜택 최종 확인 및 탈퇴 완료"
        description: "현재 소지 중인 쿠폰, 캐시, 최근 구매 내역 등이 최종 정리되었는지 확인합니다. 미정리 항목이 없다면 아래 탈퇴 동의 항목에 체크한 뒤 화면 하단의 '탈퇴하기' 버튼을 누르면 쿠팡 계정 영구 탈퇴가 완료됩니다."
        image: "/exit/coupang_11.jpg"
---

# ⚠️ 탈퇴 전 필수 주의사항 및 체크리스트

### 1. 와우 멤버십 해지 필수
- 쿠팡 와우 멤버십을 이용 중인 경우, 멤버십을 먼저 해지하지 않으면 탈퇴를 할 수 없습니다.

### 2. 잔여 쿠페이 머니 및 쿠팡 캐시 환불
- 계정에 남아있는 **쿠페이 머니**는 연결된 본인 명의 은행 계좌로 환불 신청을 진행하세요.
- **쿠팡 캐시** 및 각종 할인 쿠폰은 탈퇴 즉시 자동 소멸되며, 영구적으로 복구되지 않으므로 반드시 전액 사용 후 탈퇴를 진행하시기 바랍니다.

### 3. 진행 중인 거래 내역 확인
- 결제 완료, 배송 중, 반품/교환 처리 중인 상품이 있는 경우 탈퇴가 제한될 수 있습니다. 모든 거래 건이 '구매 확정' 또는 '처리 완료' 되었는지 확인하세요.

### 4. 재가입 및 과거 거래 기록 안내
- 탈퇴 처리 후 동일한 이메일/전화번호로 일정 기간 동안 재가입이 불가합니다.
- 기존 계정의 구매 내역, 작성한 리뷰, 영수증 증빙 자료 등은 모두 즉시 영구 삭제되어 복구가 불가능합니다.
- 전자상거래 등에서의 소비자보호에 관한 법률 등 관계 법령에 따라 거래 관련 기록은 일정 기간 동안 법적으로 분리 보관될 수 있습니다.